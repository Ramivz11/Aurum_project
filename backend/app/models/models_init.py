...
class Transferencia(Base):
    ...
    sucursal_origen_id = Column(Integer, ForeignKey("sucursales.id"), nullable=True)
...
class GananciaAjuste(Base):
    """Registra cada vez que el usuario 'limpió' (separó) la ganancia acumulada."""
    __tablename__ = "ganancia_ajuste"

    id = Column(Integer, primary_key=True, index=True)
    monto_extraido = Column(Numeric(12, 2), nullable=False)
    nota = Column(Text)
    fecha = Column(DateTime(timezone=True), server_default=func.now())
...
class MarcaConfig(Base):
    """Color y configuración visual por marca."""
    __tablename__ = "marca_config"

    id = Column(Integer, primary_key=True, index=True)
    nombre = Column(String(100), nullable=False, unique=True)
    color = Column(String(20), default="#ff9800", nullable=False)
    creado_en = Column(DateTime(timezone=True), server_default=func.now())
