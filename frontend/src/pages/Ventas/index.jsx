...
function ModalVenta({ onClose, onSaved }) {
  ...
  const filtrados = busqueda ? productos.filter(p=>p.nombre.toLowerCase().includes(busqueda.toLowerCase())) : []

  const agregar = (prod, vari) => {
    const key = `v${vari.id}`
    const ex = carrito.find(i=>i.key===key)
    if(ex) setCarrito(c=>c.map(i=>i.key===key?{...i,cantidad:i.cantidad+1}:i))
    else setCarrito(c=>[...c,{key,variante_id:vari.id,cantidad:1,precio_unitario:Number(vari.precio_venta),nombre:`${prod.nombre} — ${[vari.sabor,vari.tamanio].filter(Boolean).join('/')}`}])
    setBusqueda('')
  }
  ...
  const submit = async () => {
    ...
    setLoading(true)
    try {
      await ventasApi.crear({...form,cliente_id:form.cliente_id||null,sucursal_id:Number(form.sucursal_id),items:carrito.map(i=>({variante_id:i.variante_id,cantidad:i.cantidad,precio_unitario:i.precio_unitario}))})
      toast.success(form.estado==='confirmada'?'Venta registrada':'Pedido guardado'); onSaved(); onClose()
  ...
  return (
    ...
    >
      <div className="grid-2" style={{marginBottom:0}}>
        ...
        <div className="form-group"><label className="input-label">Sucursal *</label>
          <select className="input" value={form.sucursal_id} onChange={e=>setForm(f=>({...f,sucursal_id:e.target.value}))}>
            <option value="">Seleccionar...</option>{sucursales.map(s=><option key={s.id} value={s.id}>{s.nombre}</option>)}
          </select>
        </div>
      </div>
      <div className="form-group"><label className="input-label">Método de pago</label>
        <div style={{display:'flex',gap:8}}>{['efectivo','transferencia','tarjeta'].map(m=>(
      ...
      <div className="form-group"><label className="input-label">Buscar producto</label>
        <input className="input" value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Nombre o marca..."/>
        {filtrados.length>0 && (
          <div style={{border:'1px solid var(--border)',borderRadius:8,marginTop:4,overflow:'hidden',maxHeight:180,overflowY:'auto'}}>
            {filtrados.map(p=>p.variantes?.map(v=>(
              <div key={v.id} onClick={()=>agregar(p,v)} style={{padding:'10px 14px',cursor:'pointer',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between'}}
                onMouseEnter={e=>e.currentTarget.style.background='var(--surface2)'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                <div><div style={{fontSize:13,fontWeight:500}}>{p.nombre}</div><div style={{fontSize:11,color:'var(--text-muted)'}}>{[v.sabor,v.tamanio].filter(Boolean).join(' · ')} — Stock: {v.stock_actual}</div></div>
                <div style={{fontWeight:600}}>{formatARS(v.precio_venta)}</div>
              </div>
            )))}
          </div>
        )}
      ...
      {carrito.length>0&&(<div>
        ...
        {carrito.map((item,i)=>(
          <div key={item.key} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:'1px solid var(--border)'}}>
            <div style={{flex:1,fontSize:13}}>{item.nombre}</div>
            ...
            <input type="number" className="input" style={{width:110}} value={item.precio_unitario} onChange={e=>setCarrito(c=>c.map((x,idx)=>idx===i?{...x,precio_unitario:Number(e.target.value)}:x))}/>
            <button className="btn btn-danger btn-xs" onClick={()=>setCarrito(c=>c.filter((_,idx)=>idx!==i))}>✕</button>
          </div>
        ))}
        <div style={{textAlign:'right',marginTop:12,fontFamily:'var(--font-display)',fontSize:20,fontWeight:700,color:'var(--gold-light)'}}>Total: {formatARS(total)}</div>
      </div>)}
      {nuevoCliente&&(
...
export default function Ventas() {
  ...
  const cargar = () => { setLoading(true); ventasApi.listar({estado:filtro||undefined}).then(r=>setVentas(r.data)).finally(()=>setLoading(false)) }
  useEffect(()=>{cargar()},[filtro])

  const eliminar = async(id)=>{ await ventasApi.eliminar(id); toast.success('Eliminada'); cargar() }
  const confirmar = async(id)=>{ try{ await ventasApi.confirmar(id); toast.success('Confirmado'); cargar() }catch(e){ toast.error(e.response?.data?.detail||'Error') } }
        ...
        <div className="card"><div className="table-wrap"><table>
          ...
          <tbody>{ventas.map(v=>(
            <tr key={v.id}>
              ...
              <td><strong>{v.cliente_id?`#${v.cliente_id}`:'—'}</strong></td>
