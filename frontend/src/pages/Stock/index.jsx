...
function ModalProducto({ producto, categorias, onClose, onSaved }) {
  ...
  const [vars, setVars]   = useState(producto?.variantes?.filter(v => v.activa !== false).map(v => ({ id: v.id, sabor: v.sabor || '', tamanio: v.tamanio || '', costo: v.costo ?? '', precio_venta: v.precio_venta ?? '', stock_minimo: v.stock_minimo ?? 0, dias_duracion: v.dias_duracion ?? '' })) || [{ sabor: '', tamanio: '', costo: '', precio_venta: '', stock_minimo: 0, dias_duracion: '' }])
  const [loading, setLoading]   = useState(false)
  const [showVars, setShowVars] = useState(false)

  const norm = v => ({ sabor: v.sabor || null, tamanio: v.tamanio || null, costo: parseFloat(v.costo) || 0, precio_venta: parseFloat(v.precio_venta) || 0, stock_minimo: parseInt(v.stock_minimo) || 0, dias_duracion: v.dias_duracion ? parseInt(v.dias_duracion) : null })
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: showVars ? 12 : 0 }}>
        ...
        {showVars && <button className="btn btn-ghost btn-sm" onClick={() => setVars(v => [...v, { sabor: '', tamanio: '', costo: '', precio_venta: '', stock_minimo: 0, dias_duracion: '' }])}>+ Agregar</button>}
      </div>
